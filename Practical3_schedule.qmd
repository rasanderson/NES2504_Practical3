...
Interaction terms can be useful in both designed experiments and field surveys. They provide a way of checking whether the effects of two explanatory variables on the response are independent of each other, or alternatively whether the value of one explanatory variable alters what the other one does. Unless you want to fit curves (see @sec-curves) or have certain specific experimental designs (see @sec-blocks) it is often worth checking if your multiple explanatory variables are independent or not by using interaction terms.
...
Let's look at the example from the [Interactive Website on interaction terms (right-click and open in new tab)](https://naturalandenvironmentalscience.shinyapps.io/multiple_explan/#section-interactive-demonstration)

## Example of interactions: blood plasma calcium in rabbits
### Before you begin
1. Start RStudio in the usual way. Open your `NES2504` project from the main menu by clicking on **File -> Open project...** and navigate to your NES2504 project file and open it. **ESSENTIAL:** Check that the word "None" has been replaced by "NES2504" in the project listing at the top right of RStudio.
...
2. Using the File Explorer (main Windows menu) copy the `plasma.csv` file from `Downloads` and paste into your `NES2504` project `Data` folder.
...
* Look at your `plasma_dat` data.frame by double-clicking on its name in the RStudio Environment pane (top-right of RStudio). It has columns `calcium`, `sex`, `site`
...
Remember to enter the code for your boxplot into the bottom of `practical3.R` before running it.
 ...
 * if you were to draw a line to connect the two red boxes (females) and another line between the two blue boxes (males) the **slopes of the two lines would be not be parallel**. If when you eyeball your plot it appears that the slopes of the lines are not parallel, this suggests that there is an interaction.
 
 
### Create a linear model with an interaction
Using the information above, and your existing knowledge of `lm()`, we can create a linear model with both the `site` and `sex` main effects, and also an interaction term. The resultant ANOVA table should match the one below. **Remember to include code, blank lines and comment lines in your `practical3.R` script**.

```{r, eval=FALSE, echo=TRUE}
# Create a linear model to see if blood calcium is affected by site, sex or
# the interaction of site and sex in rabbits
calcium_lm <- lm(calcium ~ site + sex + site:sex, data=plasma_dat)
# Check the F-ratios and p-values for significance
anova(calcium_lm)
```
...
Please speak to a demonstrator if unsure, particularly as the default way in which R presents p-values is **not** an acceptable format for your reports.
...
Finally, the interaction term. As this graph is a little trickier to draw, I have included the R code. We include both the raw data, and lines connecting the means, to show the direction of change. The R code below has several lines of comments to help you understand the code, but please ask if you have questions. It also includes the `####` to auto-create a table of contents in your code. It can be useful to add explanations to your R code, so when you return to it in a few months time you understand what it is doing:

```{r warning=FALSE, message=FALSE}
# Create a full interaction plot of rabbit blood calcium data ####

# Explanation of plotting code
# gf_point() adds the raw data points
# gf_line() adds lines
# gf_line() with stat="summary"  indicates the lines should connect mean values
# gf_line() with group= ~sex     indicates separate lines for males and females
# gf_line() with colour=~sex     indicates males and females coloured different
#
...
# Plot fish data with different lines for each site in the area column ####
...
It is obvious that d13c increases with larger fish, and that concentrations appear to be higher in the SE than NW site. Are the lines parallel? The lines for the NW and SE sites look roughly parallel. But are they really? We need to do a formal statistical test using `lm()` to check.

## Analysis of covariance (ANCOVA)
When you have one continuous explanatory and one categorical explanatory, most statistics textbooks describe it as "ANCOVA". Fortunately, the method of analysis is identical to before, using the `lm` command. **BUT** a problem with having one continuous and one categorical explanatory variable is that the actual results from your `anova` function depend on the order in which you list the explanatory variables into the model. This can be resolved using the `Anova` function from the `car` library, which we will load first. 

```{r, warning=FALSE, message=FALSE}
# ANCOVA of fish data ####
# Load the car library. We need to use Anova() function as one explanatory
# variable, area, is categorical, whereas the other, standard_length, is
# continuous
library(car)
...
the interactive website [multiple explanatories (right-click and open in a new tab)](https://naturalandenvironmentalscience.shinyapps.io/multiple_explan/#section-aggh-lost-data) for more examples.

## Regression equations with continuous and categorical explanatories
Look again at the plot you created earlier: you have two straight line regression lines for areas NW and SE. Thus, there are two regression lines in the scatterplot:

$$
d13c_{NW} = intercept_{NW} + gradient_{NW} \times length_{NW} + \epsilon
$$
and
$$
d13c_{SE} = intercept_{SE} + gradient_{SE} \times length_{SE} + \epsilon
$$

Remember: the **intercept** is the value of `d13c` if your fish length is zero (not that you'll ever get a fish that tiny!!). The **gradient** is simply the change in `d13c` with each unit change of fish length. What are the values for the intercepts and gradients (slopes) in these two equations. First, what would you do if your interaction term had been significant (although we now know it isn't) which would arise if the gradients (slopes) were very different?  You can calculate them from running `summary(anr_lm)` which was your original model:

```{r echo = FALSE}
summary(anr_lm)
```

The values you are interested in are displayed under the `Estimate` column. These can be confusing to read if they are in "exponential form" with the letter `e` to indicate numbers of zeroes in the value. You may find it easier to just display the regression coefficients, in the `Estimates` column via the `coefficients()` function, but rounded to 4 significant figures:

```{r}
coef_lm <- round(coefficients(anr_lm), 4)
coef_lm
```

You can now easily construct the two regression equations, using `[]` notation to access the individual components in `coef_lm`. Site NW is treated as the "baseline", simply because "N" comes before the "S" of SE in the alphabet. You now have enough information to calculate the regression lines.

### Area NW
This is the simplest one, as the values can be read directly from the set of coefficients.

Intercept = `(Intercept)` = `coef_lm[1]` = `r coef_lm[1]`

Gradient = `standard_length` = `coef_lm[2]` = `r coef_lm[2]`

$$
d13c_{NW} = `r coef_lm[1]` + `r coef_lm[2]` \times length_{NW} + \epsilon
$$

### Area SE
This is trickier. The `areaSE` coefficient shows the difference between the intercept of the SE site vs the NW site. Here it is a positive number, so when the `standard_length` is zero, site SE has a higher `d13c` which you can see easily on your graph. The interaction term `standard_length:areaSE` is the difference in the gradient of the line for area SE and area NW. This is tiny, and indeed looking at your plot you can see the two lines appear parallel, and you already know that there is no significant difference in the gradients. Nevertheless, sometimes you will have a real difference in gradients, with a significant interaction, so it is useful to know how to calculate the second regression line.


Intercept = `(Intercept) + standard_length` = `coef_lm[1] + coef_lm[3]` = `r coef_lm[1] + coef_lm[3]`

Gradient = `standard_length + standard_length:areaSE` = `coef_lm[2] + coef_lm[4]` = `r coef_lm[2] + coef_lm[4]`

$$
d13c_{SE} = `r coef_lm[1] + coef_lm[3]` + `r coef_lm[2] + coef_lm[4]` \times length_{SE} + \epsilon
$$

In reality, you have already found that your second model, `anr_lm2`, which does not have an interaction term, is the better one to use for these data as the slopes of the lines are parallel. Therefore you should use `summary(anr_lm2)` if you want to obtain the correct equations for the two lines. They will have different intercepts, but the same gradient. See if you can work them out yourself.
...
Let us build on the example at the [Interactive Website on Curve fitting (right-click and open in new tab)](https://naturalandenvironmentalscience.shinyapps.io/multiple_explan/#section-fitting-curves) for growth of the grass _Lolium perenne_ in soils of different water content. You will need to first:
...
Use the `head()` function to look at the first few rows, or double-click on `lolium_dat` in the "Environment" pane (top-right of RStudio) to view the whole table of data. Next, produce a scatterplot of the data with the following code. Remember to add blank lines to break up "paragraphs" of code to make it readable:
...
```{r, echo=TRUE}
# Create a scatterplot of Lolium growth data ####
...
We can actually produce 4 "diagnostic" plots quite easily with the `plot()` function, writing `plot(lolium_lm1)`. Only the first two are really useful here. The `plot()` function extracts the residuals and creates a QQ plot automatically. All the plots look at "Residuals". Remember residuals are the differences between your fitted straight line and the observed points. Remember to hit the "Enter" key in the Console window (bottom left of RStudio) to view both diagnostic plots:
...
```{r, echo=TRUE}
# Lolium scatterplot with badly fitted straight line
gf_point(growth ~ water, data=lolium_dat) %>% 
  gf_labs(x = "Soil water content (%)", y="Growth rate") %>% 
  gf_lm() %>% 
  gf_theme(theme_classic())
...
```{r}
# Lolium scatterplot with quadratic curve plus 95% confidence intervals
gf_point(growth ~ water, data = lolium_dat) %>% 
...
## Randomised block design {#sec-blocks}
